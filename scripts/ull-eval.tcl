...
set expID 1; # [expr int([lindex $argv 1])]
...
set sender_type [lindex $argv 0];
if {([string compare $sender_type "both"] == 0)} {

} elseif {([string compare $sender_type "legacy"] == 0)} {

} elseif {([string compare $sender_type "new"] == 0)} {

} else {
    puts "Invalid sender_type: Must be one of both, legacy, new"
    exit 1
}
...
proc print_queue_stat {} {
    global left_bcm sender_type
    if {([string compare $sender_type "legacy"] == 0)} {
        puts "Legacy Queue Max buildup : [eval $left_bcm get-max-queue-buildup 2] bytes"
    } else {
        puts "FlexPass Main Queue Max buildup : [eval $left_bcm get-max-queue-buildup 1] bytes"
    }
}

$ns at 9.9 print_queue_stat
$ns at 10 "finish"
$ns run
